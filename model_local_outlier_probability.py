...
outlier_path = Path(
    "Data/Input_Data/dataframe_with_outliers_3std_4.16%.csv").absolute()
...
def hyper_parameter_testing(dataframes: dict, extents: list, n_neighbors: list, critical_values: list):
    columns = ["e", "n", "%"]
        ...
        for n in n_neighbors:  # hyper parameter 2
            for value in critical_values:  # hyper parameter 3
                ...
                for letter, dataframe in dataframes.items():
                    ...
                    dataframe_for_model = dataframe.iloc[:, 1:15]
                    ...
                    for score in scores:
                        if score >= value:  # consider it as an outlier, when score > critical value
                            part_outlier_number += 1  # count the outliers per letter
                            part_outlier_list.append(1)  # mark as outlier
                        else:
