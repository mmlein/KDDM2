...
def cross_validation(params: dict, dataset_complete: pd.DataFrame, write_to_csv: bool = False) -> tuple[pd.DataFrame,
    ...
    if write_to_csv == True:
        path_abs = Path(
            "Data/Output_Data/02_Results_Ifor/Ifor_abs.csv").absolute()
        path_rel = Path(
            "Data/Output_Data/02_Results_Ifor/Ifor_rel.csv").absolute()
...
def main():
    ...
    cross_validation(hyper_params, letter_df_complete, True)
